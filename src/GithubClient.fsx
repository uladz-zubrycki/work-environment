...
type SortDirection = Asc | Desc with override x.ToString() = UnionUtils.toString x
...
module GetPullRequests = 
    ...
    type PullRequestState = Open | Closed | All with override x.ToString() = UnionUtils.toString x
    type PullRequestHead = { User: string; RefName: string }
    type PullRequestSortType = Created | Updated | Popularity | LongRunning with override x.ToString() = UnionUtils.toString x
    ...
    type GetPullRequestsCommand = 
        static member Execute(repoInfo: RepoInfo)
            ...
            | 200, Some(responseBody) -> Response.Parse responseBody
            | 400, Some(responseBody) -> 
                let error = HttpErrorResponse.Parse responseBody
                error.Message 
                |> sprintf "Bad request. Error: '%s'" 
                |> apiFailure
            | 401, _ -> apiFailure "Github communication error: Bad credentials."
            | 403, _ -> apiFailure "Github communication error: Forbidden."
            | statusCode, maybeResponseBody -> 
                (statusCode, if maybeResponseBody.IsSome then sprintf ", response body: '%s'" maybeResponseBody.Value else "")
                ||> sprintf "Unexpected github communication error. Status code: '%d'%s." 
                |> apiFailure
            | _ -> apiFailure "Unexpected github communication error." 
